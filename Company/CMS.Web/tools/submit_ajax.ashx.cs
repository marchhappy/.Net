...
namespace CMS.Web.tools
{
    /// <summary>
    /// submit_ajax 的摘要说明
    /// Update  2015/11/21  对检测用户登录的优化
    /// Update  2015/11/26  对获取用户登录信息进行优化（groupID需要重新登录才会更新）
    /// </summary>
    public class submit_ajax : IHttpHandler, IRequiresSessionState
    {
        ...
        private void comment_add(HttpContext context)
        {
            ...
            Model.users userModel = new Web.UI.BasePage().GetUserInfo(false);
            if (userModel != null)
            {
        ...
        private void user_login(HttpContext context)
        {
            ...
            //检查当前用户名尝试次数
            Model.users_login modellogin = CacheHelperRedis.Get<Model.users_login>(DTKeys.COOKIE_USER_LOGIN_CHECK + username);
            if (modellogin != null)
            {
                //检查错误次数总数是否大于10次
                if (modellogin.login_wrong_time > 10)
                {
                    context.Response.Write("{\"status\": 0, \"msg\": \"错误提示：该用户尝试登录次数已达最大值，您可以尝试找回密码或者联系客服申述账号！\"}");
                    return;
                }
                else
                {
                    //检查尝试时间
                    TimeSpan tsCheck = DateTime.Now - modellogin.login_time;
                    int needMinutes = (int)Math.Pow((double)5, (double)((modellogin.login_wrong_time - 3) > 0 ? modellogin.login_wrong_time - 3 : 0));
                    if ((int)tsCheck.TotalMinutes < needMinutes)
                    {
                        context.Response.Write("{\"status\": 0, \"msg\": \"错误提示：请稍后再尝试登录，或者您可以尝试找回密码或者联系客服申述账号！\"}");
                        return;
                    }
                }
            }
            ...
            if (model == null)
            {
                ...
                if (modellogin == null)
                {
                    modellogin = new Model.users_login();
                    modellogin.login_wrong_time = 0;
                }
                modellogin.login_ip = DTRequest.GetIP();
                modellogin.login_time = DateTime.Now;
                modellogin.login_wrong_time = modellogin.login_wrong_time + 1;
                CacheHelperRedis.Insert(DTKeys.COOKIE_USER_LOGIN_CHECK + username, modellogin, DateTime.Now.AddHours(24));
                //提示错误信息
            ...
            {
                //删除登录信息记录
                CacheHelperRedis.Remove(DTKeys.COOKIE_USER_LOGIN_CHECK + username);
            }
            //检查用户是否通过验证
            ...
            //记录登录状态(2小时内有效)
            model.model_login = new Model.users_login();
            model.model_login.login_ip = DTRequest.GetIP();
            model.model_login.login_time = DateTime.Now;
            model.model_login.login_wrong_time = 0;
            CacheHelperRedis.Insert(sessionID, model, DateTime.Now.AddHours(2));
            //记住登录状态下次自动登录
            if (remember.ToLower() == "true")
            {
                Utils.WriteCookie(DTKeys.COOKIE_USER_INFO_REMEMBER, sessionID, 43200);
            }
            else
            {
                Utils.WriteCookie(DTKeys.COOKIE_USER_INFO_REMEMBER, sessionID);
            }
            //写入登录日志
        ...
        {
            //检查用户是否登录
            Model.users model = new BasePage().GetUserInfo(false);
            if (model == null)
            {
        ...
        private void user_avatar_crop(HttpContext context)
        {
            ...
            {
                Model.files modelfile = new BLL.files().GetModel(fileName.Split('?')[fileName.Split('?').Length - 1].ToString().Trim());
                if (modelfile != null)
                {
                    fileName = modelfile.file_path;
                }
            }
            //检查参数
            ...
            string fileHash = "";
            bool result = upFiles.cropSaveAs(fileName, fileName, 180, 180, w, h, x1, y1, out fileHash);
            if (!result)
            {
        ...
        {
            //检查用户是否登录
            Model.users userModel = new BasePage().GetUserInfo(false);
        ...
        private void cart_goods_add(HttpContext context)
        {
            ...
            //查找会员组
            int group_id = 0;
            Model.users groupModel = new Web.UI.BasePage().GetUserInfo(false);
            if (groupModel != null)
            {
        ...
        private void order_save(HttpContext context)
        {
            ...
            {
                //检查库存数量
                if (new BLL.article().GetStockQuantity(item.article_id, item.goods_id) < item.quantity)
                {
