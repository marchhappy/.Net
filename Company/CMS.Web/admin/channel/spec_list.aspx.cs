    ...
    public partial class spec_list : Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ...
                RptBind("parent_id=0" + CombSqlTxt(keywords), "sort_id asc,id desc");
            }
        }
        ...
        {
            int _pagesize;
            if (int.TryParse(Utils.GetCookie("spec_page_size", "CMSPage"), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    return _pagesize;
        ...
        {
            Response.Redirect(Utils.CombUrlTxt("spec_list.aspx", "keywords={0}", txtKeywords.Text));
        }
        ...
        protected void txtPageNum_TextChanged(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    Utils.WriteCookie("spec_page_size", "CMSPage", _pagesize.ToString(), 14400);
                }
            }
            Response.Redirect(Utils.CombUrlTxt("spec_list.aspx", "keywords={0}", this.keywords));
        }
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            JscriptMsg("保存排序成功！", Utils.CombUrlTxt("spec_list.aspx", "keywords={0}", this.keywords));
        }

        //批量删除
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            ChkAdminLevel("sys_goods_spec", DTEnums.ActionEnum.Delete.ToString()); //检查权限
            BLL.article_spec bll = new BLL.article_spec();
            for (int i = 0; i < rptList.Items.Count; i++)
            {
                int id = Convert.ToInt32(((HiddenField)rptList.Items[i].FindControl("hidId")).Value);
            ...
            JscriptMsg("删除规格成功！", Utils.CombUrlTxt("spec_list.aspx", "keywords={0}", this.keywords));
        }
