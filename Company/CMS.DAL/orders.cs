            ...
            if (model.order_goods != null)
            {
                ...
                foreach (Model.order_goods modelt in model.order_goods)
                {
                    ...
                    if (modelt.goods_id > 0)
                    {
                        ...
                        strSql3.Append("update " + databaseprefix + "article_goods set ");
                        strSql3.Append("stock_quantity=stock_quantity-@stock_quantity where id=@goods_id and article_id=@article_id");
                        ...
                        parameters3[2].Value = modelt.article_id;
                        cmd = new CommandInfo(strSql3.ToString(), parameters3);
                        sqllist.Add(cmd);

                        strSql4 = new StringBuilder();
                        strSql4.Append("update " + databaseprefix + "article_attribute_value set ");
                        strSql4.Append("stock_quantity=(select sum(stock_quantity) from " + databaseprefix + "article_goods where article_id=@article_id)");
